...
// Romo.UI.PopoverStack models a stack of Romo.Popover component instances. The
// popovers (modals, dropdowns, etc) can be nested such that, for example, a
...
// '[data-romo-dropdown-auto-init]', etc).
Romo.define('Romo.UI.PopoverStack', function() {
  return class {
    ...
    addPopover(popoverDOM, boundOpenFn, boundCloseFn, boundPlaceFn) {
      ...
      Romo.pushFn(Romo.bind(function() {
        this.items.push(
          new Romo.UI.PopoverStack.Item(popoverDOM, boundCloseFn, boundPlaceFn)
        )
        boundOpenFn()
      }, this))
    }
    ...
    placeAllPopovers(includingFixed) {
      this
        .items
        .filter(function(item) {
          return includingFixed || item.getPositionStyle() !== 'fixed'
        })
    ...
    _closeTop() {
      ...
      if (this.items.length > 0) {
        ...
        this.bodyDOM.trigger(
          'Romo.UI.PopoverStack:popoverClosed',
          [this, item.popoverDOM],
        )
        item.trigger('Romo.UI.PopoverStack:popoverClosed', [this])
      }
      return item
    }
    ...
    _onBodyKeyUp(e) {
      ...
      if (e.keyCode === 27 /* Esc */) {
        ...
        if (closedItem) {
          closedItem.trigger('Romo.UI.PopoverStack:popoverClosedByEsc', [this])
        }
      }
    }
