Romo.define('Romo.UI.Popover', function() {
  return class {
    ...
    get romoXHR() {
      return Romo.memoize('romoXHR', function() {
        return this._bindXHR()
      })
    }

    get dom() {
      return Romo.memoize('dom', function() {
...
`
    ...
    get appendReceiverDOM() {
      return Romo.memoize('appendReceiverDOM', function() {
    ...
    doClose() {
      Romo.popoverStack.closeThru(this.dom)
    }
    ...
    setContentMaxHeightPx(heightPx) {
      const deltaHeightPx =
    ...
    _bindPopover() {
      ...
      Romo.pushFn(Romo.bind(function() {
        ...
        this.spinnerDOM.on(
          'Romo.Spinner:started',
          Romo.bind(function(e, romoSpinner) {
            this.spinnerDOM.show()
            this._place()
          }, this)
        )
        this.spinnerDOM.on(
          'Romo.Spinner:stopped',
          Romo.bind(function(e, romoSpinner) {
    ...
    _bindBody() {
      ...
      this.contentDOM =
        this.bodyDOM.find(`[data-${this.attrPrefix}-popover-content]`).last()
      ...
      this.dom.find(`[data-${this.attrPrefix}-popover-close]`).on(
        'click',
        Romo.bind(this.popoverOwner.doClosePopover, this.popoverOwner)
      )
    ...
    _resetBodyContent() {
      ...
      this
        .dom
        .find(`[data-${this.attrPrefix}-popover-close]`)
        .off(
          'click',
          Romo.bind(this.popoverOwner.doClosePopover, this.popoverOwner)
        )
    ...
    _onScrollableParentsScroll(e) {
      Romo.popoverStack.placeAllPopovers()
    }
